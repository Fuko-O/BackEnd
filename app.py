...
# On importe 'render_template' pour servir index.html
...
# On lit la clé depuis les variables d'environnement (plus sécurisé)
API_KEY = os.environ.get("GEMINI_API_KEY") 

if not API_KEY:
    print("ERREUR FATALE : La variable d'environnement GEMINI_API_KEY n'est pas définie.")

IA_MODEL_NAME = "gemini-pro-latest" 
# On garde l'URL v1beta qui marche pour ta clé
IA_API_URL = f"https://generativelanguage.googleapis.com/v1beta/models/{IA_MODEL_NAME}:generateContent?key={API_KEY}"

# On a supprimé la ligne "genai.configure..." qui était une erreur
...
def api_learn_rule():
    ...
    success = sauvegarder_regle_en_bdd(
        mot_cle=mot_cle,
...
# --- 5. Lancement (Modifié pour Render) ---
